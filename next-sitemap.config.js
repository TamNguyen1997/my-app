...
async function generateSitemap() {
  ...
  fs.writeFileSync(__dirname + '/public/sitemap-0.xml', `
...
function convertToXmlProductSiteMap(products, priority, changefreq) {
  ...
  <urlset xmlns="http://www.sitemaps.org/schemas/sitemap/${changefreq}">
  ${products.map(p => `
    <url>
      <loc>${siteUrl}/${p.subCate.slug}/${p.slug}</loc>
      <lastmod>${p.updatedAt.toISOString()}</lastmod>
      <changefreq>${priority}</changefreq>
      <priority>${changefreq}</priority>
    </url>`).join('')}
  </urlset>`;
}
...
function convertToXmlCategoriesSiteMap(categories, priority, changefreq) {
  ...
  ${categories.map(cate => `
    <url>
      <loc>${siteUrl}/${cate.slug}</loc>
      <lastmod>${cate.updatedAt.toISOString()}</lastmod>
      <changefreq>${priority}</changefreq>
      <priority>${changefreq}</priority>
    </url>`).join('')}
  </urlset>`;
...
function convertToXmlBLogsSiteMap(blogs, priority = 0.9, changefreq) {
  ...
  ${blogs.map(blog => `
    <url>
      ...
      <lastmod>${blog.modified || new Date().toISOString()}</lastmod>
      <changefreq>${priority}</changefreq>
      <priority>${changefreq}</priority>
    </url>`).join('')}
  </urlset>`;
