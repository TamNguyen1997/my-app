...
const Category = () => {
  ...
  const getCategories = () => {
    ...
    fetch(`/api/categories/?size=${10}&page=${page}&${queryString}&includeImage=true`).then(async res => {
                ...
                <>
                  ...
                  <ModalBody>
                    ...
                    <div className="flex gap-2">
                      <Select
                        label="Loại"
                        defaultSelectedKeys={new Set([selectedCate.type])}
                        onSelectionChange={(value) => {
                          if (value.values().next().value === "CATE") {
                            setSelectedCate(Object.assign({}, selectedCate, { cateId: null }))
                          }
