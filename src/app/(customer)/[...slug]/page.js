...
const Page = async ({ params }) => {
  ...
  if (categorySlug && productSlug) {
    ...
    if (product) {
      let description = "";
      const productPostResponse = await fetch(`${process.env.WORDPRESS_URL}/wp-json/wp/v2/posts/?slug=${product.slug}&categories=${process.env.NEXT_PUBLIC_WORDPRESS_PRODUCT_CATEGORY_ID}`);
      if (productPostResponse.ok) {
        const productPost = await productPostResponse.json();
        description = productPost[0]?.content?.rendered;
      }
      return <ProductDetail id={product.slug} product={product} description={description} images={product.product_on_image?.map(item => item.image)} />
    }
  }
  notFound()
}
