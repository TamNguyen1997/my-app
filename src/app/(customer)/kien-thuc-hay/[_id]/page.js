...
export async function generateMetadata({ params }) {
  const res = await fetch(`${process.env.NEXT_PUBLIC_WORDPRESS_URL}/wp-json/wp/v2/posts/?slug=${params._id}&_embed&categories_exclude=${parseInt(process.env.NEXT_PUBLIC_WORDPRESS_PRODUCT_CATEGORY_ID)}`.trim())
  if (!res.ok) {
    return {}
  }
