...
export async function POST(req) {
  try {
    ...
    while (check) {
      ...
      const categoryExist = await db.category.findUnique({ where: { slug: slug } })
      if (categoryExist != null) {
        fs.appendFile(`./public/log/import-category.txt`, slug + '\n', function (err) {
          if (err) throw err;
        });
        isInsertLog = true;
        index++
    ...
    if (isInsertLog) {
      return NextResponse.json({ message: "Please check log/import-category.txt" }, { status: 400 });
    }
