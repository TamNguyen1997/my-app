...
const Payment = () => {
  ...
  useEffect(() => {
    fetch(`/api/courier/get-cities`)
      .then(res => res.json())
      .then(data => setShipping(prevState => ({ ...prevState, cities: data.data })));
  }, []);
  ...
  const handleCityChange = (value) => {
    ...
    fetch(`/api/courier/get-districts?provinceId=${provinceId}`)
      .then(res => res.json())
      .then(data => setShipping(prevState => ({ ...prevState, districts: data.data })));
  };
  ...
  const handleDistrictChange = (value) => {
    ...
    fetch(`/api/courier/get-wards?districtId=${districtId}`)
      .then(res => res.json())
      .then(data => setShipping(prevState => ({ ...prevState, wards: data.data })));
  };
            ...
            <div className="space-y-4 rounded-lg border border-gray-200 bg-white p-4 shadow-sm sm:p-6">
              <div className="space-y-2">
                ...
                <div className="grid sm:grid-cols-3 gap-2">
                  ...
                  >
                    {cities.map(item => (
                      <SelectItem key={item.PROVINCE_ID}>
                        {item.PROVINCE_NAME}
                      </SelectItem>
                    ))}
                  </Select>
                  ...
                  >
                    {districts.map(item => (
                      <SelectItem key={item.DISTRICT_ID} className="capitalize">
                  ...
                  >
                    {wards.map(item => (
                      <SelectItem key={item.WARDS_ID} className="capitalize">
                ...
                <div className="!my-3">
                  <Checkbox isSelected={isVATActive} onValueChange={setIsVATActive}>
                    Xuất hoá đơn khách hàng doanh nghiệp
                  </Checkbox>
                  {isVATActive && (
                    <div className="space-y-2 !mt-3 !mb-6">
                      <p className="text-gray-500">
                        ...
                        hoặc điện thoại: <Link href="tel:0902802979">090 280 2979</Link> nếu quý khách đã thanh toán tiền hàng và chưa nhận được hóa đơn trong vòng 3 ngày kể
                ...
                <div className="flex flex-wrap gap-5">
                  ...
                  <Checkbox
                    ...
                    radius="full"
                    value="VIETQR"
                    isSelected={selected === "VIETQR"}
                    onValueChange={() => setSelected("VIETQR")}
                  >
                    Chuyển khoản
                  </Checkbox>
                </div>
